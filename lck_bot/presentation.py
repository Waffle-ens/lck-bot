...
def render_result_game(report: GameReport) -> discord.Embed:
    embed = discord.Embed(
        title=f"{report.set_number}세트 상세",
    ...
    for team in _teams(report):
        players = [player for player in report.players if player.team == team]
        lines = [
            f"`{player.role}` {player.player} ({player.champion}) { _kda(player.kills, player.deaths, player.assists) }"
